// ==================================================================
// SCRIPT SIMPLIFICADO PARA A PÁGINA DE VOLUNTÁRIOS ATIVOS - VERSÃO DE DIAGNÓSTICO
// ==================================================================
...
const mockVolunteers = [
    { id: 'mock1', registerName: 'Carlos Miranda (Mock)', email: 'carlos.mock@surf.com', active: true, gender: 'Masculino', userRole: "2" },
    { id: 'mock2', registerName: 'Ana Castilho (Mock)', email: 'ana.mock@surf.com', active: true, gender: 'Feminino', userRole: "2" }
];

// --- INICIALIZAÇÃO ---
document.addEventListener('DOMContentLoaded', () => {
    // Sempre fazer uma nova consulta ao backend quando a página é carregada
    localStorage.removeItem('cachedActiveVolunteers');
    
    // Verifica se precisamos atualizar os dados (indicado pela página de inativos)
    if (localStorage.getItem('shouldRefreshActiveVolunteers') === 'true') {
        console.log("Atualizando lista de voluntários ativos após reativação...");
        localStorage.removeItem('shouldRefreshActiveVolunteers');
    }
...
async function loadAndRenderVolunteers() {
    try {
        ...
        if (!token) {
            ...
            // Usar o endpoint específico para voluntários ativos, se existir
            ...
            // Filtra apenas voluntários ativos (userRole = 2 para voluntários)
            const realActiveVolunteers = data.filter(user => 
                (user.userRole === "2" || user.userRole === 2) && 
                user.active !== false
            );
            console.log("Voluntários carregados da API:", realActiveVolunteers.length);
            
            // Combinar voluntários mockados com voluntários reais
            allVolunteers = [...mockVolunteers, ...realActiveVolunteers];
        }
    } catch (error) {
...
function createVolunteerRowHTML(volunteer) {
    ...
    const statusClass = (volunteer.active === false) ? 'inactive' : 'active';
    const statusText = (volunteer.active === false) ? 'Inativo' : 'Ativo';
...
function renderTable(volunteersToRender = filteredVolunteers) {
    ...
    if (badge) badge.textContent = `${volunteersToRender.length} Cadastros Ativos`;
...
Status: ${volunteer.active === false ? 'Inativo' : 'Ativo'}
    `;
...
function editUser(userId) {
    const volunteerToEdit = allVolunteers.find(v => v.id == userId);
    if (volunteerToEdit) {
        sessionStorage.setItem('voluntarioParaEditar', JSON.stringify(volunteerToEdit));
        window.location.href = `../cadastro-voluntario/cadastro-voluntario.html`;
    }
}
...
async function deactivateUser(userId) {
    ...
    if (confirm(`Deseja realmente desativar este voluntário?`)) {
        try {
            const token = localStorage.getItem("authToken");
            if (!token) {
                alert("Você precisa estar logado para desativar voluntários.");
                return;
            }
            ...
            // Atualiza o voluntário com o campo active = false
            ...
            // Atualiza a lista de voluntários removendo o desativado
            ...
            localStorage.setItem('shouldRefreshInactiveVolunteers', 'true');
            
            alert("Voluntário desativado com sucesso! Ele agora aparecerá na lista de voluntários inativos.");
            renderPage();
        } catch (error) {
            console.error("Erro ao desativar voluntário:", error);
            alert("Erro ao desativar voluntário. Por favor, tente novamente. Detalhes: " + error.message);
        }
    }
}
